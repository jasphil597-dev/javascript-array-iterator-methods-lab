...
veryOldInventors = inventors.filter((inventors) => {
	if (inventors.year > 1500 && inventors.year < 1600) return inventors;
});
...
inventorNames = inventors.map(({ inventor, first, last }) => ({
	first,
	last,
}));
...
// Complete the exercise in the space below:
isAdultPresent = people.some((people) => {
	return isAdultPresent !== 18;
});
...
isEveryone19OrOlder = devs.every((dev) => {
	return dev.isEveryone19OrOlder > 19;
});
